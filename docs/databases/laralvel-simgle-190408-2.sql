...
CREATE TABLE `stock` (
  ...
  `inv`         int(11)                                 NOT NULL COMMENT '实际库存/库存数量',
  `category_id` int(11)                                 NOT NULL COMMENT '商品类别id',
  `status`      tinyint(4)                              NOT NULL COMMENT '商品库存状态: 0下架, 1上架',
  `stock_price` int(11)                                 NOT NULL COMMENT '库存单价/库存成本',
  `barcode`     varchar(255) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '商品编码/第三方编码',
